...
const DashboardPage = () => {
  ...
  return (
    <div className="min-h-screen p-4 md:p-6 bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950">
      <div className="flex flex-col md:flex-row gap-4">
        <Sidebar activeTab={activeTab} setActiveTab={setActiveTab} />
        <main className="flex-1 space-y-4">
          <div className="bg-white/85 backdrop-blur rounded-2xl p-5 shadow-xl border border-white/70 flex items-center justify-between animate-rise-in">
            <div>
              <h1 className="text-2xl font-semibold text-slate-800">Welcome, {company.name}</h1>
              <p className="text-sm text-slate-600">Manage textile operations in one place.</p>
            </div>
          ...
          {activeTab === 'materials' && (
            <section className="bg-white/85 backdrop-blur rounded-2xl p-5 shadow-xl border border-white/70 space-y-3">
              <h2 className="text-lg font-semibold">Raw Materials</h2>
              <div className="grid md:grid-cols-4 gap-2">
                {['name', 'quantity', 'cost_per_unit'].map((field) => (
                  <input key={field} className="border border-slate-200 p-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-300" placeholder={field} value={forms.materials[field]} onChange={(e) => setForms((p) => ({ ...p, materials: { ...p.materials, [field]: e.target.value } }))} />
                ))}
                <button className="primary-btn rounded-lg px-3" onClick={() => onSubmit('materials', '/api/materials')} disabled={loading}>Add</button>
              </div>
              <div className="overflow-auto">
                <table className="w-full text-sm"><thead><tr className="text-left text-slate-600"><th>Name</th><th>Qty</th><th>CPU</th></tr></thead><tbody>{data.materials.map((m) => <tr key={m.id}><td>{m.name}</td><td>{m.quantity}</td><td>{m.cost_per_unit}</td></tr>)}</tbody></table>
              </div>
            </section>
          )}
            ...
            <section className="bg-white/85 backdrop-blur rounded-2xl p-5 shadow-xl border border-white/70 space-y-3">
              ...
              <div className="grid md:grid-cols-6 gap-2">
                {['product_name', 'quantity', 'cost', 'date'].map((field) => (
                  <input key={field} type={field === 'date' ? 'date' : 'text'} className="border border-slate-200 p-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-300" placeholder={field} value={forms.production[field]} onChange={(e) => setForms((p) => ({ ...p, production: { ...p.production, [field]: e.target.value } }))} />
                ))}
                <button className="primary-btn rounded-lg px-3" onClick={() => onSubmit('production', '/api/production')} disabled={loading}>Add</button>
              </div>
            ...
            <section className="bg-white/85 backdrop-blur rounded-2xl p-5 shadow-xl border border-white/70 space-y-3">
              ...
              <div className="grid md:grid-cols-7 gap-2">
                {['buyer_name', 'location', 'quantity', 'selling_price', 'date'].map((field) => (
                  <input key={field} type={field === 'date' ? 'date' : 'text'} className="border border-slate-200 p-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-300" placeholder={field} value={forms.sales[field]} onChange={(e) => setForms((p) => ({ ...p, sales: { ...p.sales, [field]: e.target.value } }))} />
                ))}
                <button className="primary-btn rounded-lg px-3" onClick={() => onSubmit('sales', '/api/sales')} disabled={loading}>Add</button>
              </div>
          ...
          {activeTab === 'analytics' && data.analytics && (
            <section className="grid lg:grid-cols-2 gap-4">
              <div className="bg-white/85 backdrop-blur rounded-2xl p-4 shadow-xl border border-white/70 h-80">
              ...
              <div className="bg-white/85 backdrop-blur rounded-2xl p-4 shadow-xl border border-white/70 lg:col-span-2">
                <h3 className="font-semibold mb-2">Low Stock Warnings</h3>
                {data.lowStock.length === 0 ? <p className="text-green-600">No low stock alerts 🎉</p> : <ul className="list-disc pl-5 text-slate-700">{data.lowStock.map((item) => <li key={item.id}>{item.name} ({item.quantity})</li>)}</ul>}
              </div>
            </section>
          )}

          {activeTab === 'insights' && (
            <section className="bg-white/85 backdrop-blur rounded-2xl p-5 shadow-xl border border-white/70 space-y-4">
